{*************************************************************************************************************************************}
{* IMPORTANT : If you change some data here, you have to report these changes in the ./klarnacheckout-json.js (to let ajaxCart available) *}
{*************************************************************************************************************************************}
{if $ajax_allowed}
<script type="text/javascript">
var CUSTOMIZE_TEXTFIELD = {$CUSTOMIZE_TEXTFIELD};
var customizationIdMessage = '{l s='Customization #' mod='klarnacheckout' js=1}';
var removingLinkText = '{l s='remove this product from my cart' mod='klarnacheckout' js=1}';
var freeShippingTranslation = '{l s='Free shipping!' mod='klarnacheckout' js=1}';
var freeProductTranslation = '{l s='Free!' mod='klarnacheckout' js=1}';
var delete_txt = '{l s='Delete' mod='klarnacheckout'}';
var img_dir = '{$img_dir}';
</script>
{/if}
...
<div id="cart_block" class="block exclusive">
	<h4>
		<a href="{$link->getPageLink("$order_process", true)}">{l s='Cart' mod='klarnacheckout'}</a>
		{if $ajax_allowed}
	...
	<div id="cart_block_summary" class="{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)}collapsed{else}expanded{/if}">
		...
		<span class="ajax_cart_product_txt_s" {if $cart_qties <= 1}style="display:none"{/if}>{l s='products' mod='klarnacheckout'}</span>
		<span class="ajax_cart_product_txt" {if $cart_qties > 1}style="display:none"{/if}>{l s='product' mod='klarnacheckout'}</span>
		...
		<span class="ajax_cart_no_product" {if $cart_qties != 0}style="display:none"{/if}>{l s='(empty)' mod='klarnacheckout'}</span>
	</div>
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dd id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)}" title="{l s='Product detail' mod='klarnacheckout'}">{$product.attributes_small}</a>
			{/if}
				...
				<ul class="cart_block_customizations" id="customization_{$productId}_{$productAttributeId}">
					{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
						<li name="customization">
							...
							{else}
							{l s='Customization #%d:' sprintf=$id_customization|intval mod='klarnacheckout'}
							{/if}
						</li>
					{/foreach}
				</ul>
	...
	{/if}
		<p {if $products}class="hidden"{/if} id="cart_block_no_products">{l s='No products' mod='klarnacheckout'}</p>
	{if $discounts|@count > 0}
						...
						<tr class="bloc_cart_voucher" id="bloc_cart_voucher_{$discount.id_discount}">
							...
							<td class="delete">
								{if strlen($discount.code)}
									<a class="delete_voucher" href="{$link->getPageLink('$order_process', true)}?deleteDiscount={$discount.id_discount}" title="{l s='Delete' mod='klarnacheckout'}"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete' mod='klarnacheckout'}" class="icon" /></a>
								{/if}
							</td>
						</tr>
						{/if}
		...
		<p id="cart-prices">
			...
			<span>{l s='Shipping' mod='klarnacheckout'}</span>
			<br/>
			{if $show_wrapping}
				...
				<span>{l s='Wrapping' mod='klarnacheckout'}</span>
				<br/>
			{/if}
			{if $show_tax && isset($tax_cost)}
				<span id="cart_block_tax_cost" class="price ajax_cart_tax_cost">{$tax_cost}</span>
				<span>{l s='Tax' mod='klarnacheckout'}</span>
				<br/>
			{/if}
			<span id="cart_block_total" class="price ajax_block_cart_total">{$total}</span>
			<span>{l s='Total' mod='klarnacheckout'}</span>
		</p>
		{if $use_taxes && $display_tax_label == 1 && $show_tax}
			{if $priceDisplay == 0}
				<p id="cart-price-precisions">
					{l s='Prices are tax included' mod='klarnacheckout'}
				</p>
			{/if}
			{if $priceDisplay == 1}
				<p id="cart-price-precisions">
					{l s='Prices are tax excluded' mod='klarnacheckout'}
				</p>
			{/if}
		{/if}
		<p id="cart-buttons">
		...
		<div class="klarnabadge">
<img src="https://cdn.klarna.com/public/images/SE/badges/v1/checkout/SE_checkout_badge_banner.png?width=185" title="{l s='Pay trough Klarna Checkout' mod='klarnacheckout'}">
</div>
	</div>
	</div>
</div>
